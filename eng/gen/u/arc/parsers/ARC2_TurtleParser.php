    ...
    while ($d = $this->reader->readStream(0)) {
      ...
      do {
        ...
        if ($prologue_done && (list($sub_r, $sub_v, $more_triples, $sub_v2) = $this->xTriplesBlock($sub_v)) && is_array($sub_r)) {
          $proceed = 1;
          $loops = 0;
					//FRI: prepare SOLR triples docs to be updated all in once
          foreach ($sub_r as $t) {
						$solr_docs_here = $this->addT($t);
						if (is_array($solr_docs_here)&&count($solr_docs_here)) 
								$solr_docs = array_merge($solr_docs,$solr_docs_here);;
          }
        }
      } while ($proceed);
      $loops++;
    ...
    if ($this->unparsed_code && !$this->getErrors()) {
      ...
      if (trim($rest)) $this->addError('Could not parse "' . $rest . '"');
		...
		if ($this->use_arc_solr)
			arc_solr_bridge_index_solr_docs($this,$solr_docs);
    return $this->done();
  }
