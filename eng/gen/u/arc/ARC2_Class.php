 ...
 * @package ARC2
 * @author: Fabio Ricci (fabio.ricci@ggaweb.ch) changed some parts to work with SOLR
 */
...
class ARC2_Class {
  ...
  function __init() {/* base, time_limit */
    ...
    $this->has_pcre_unicode = @preg_match('/\pL/u', 'test');/* \pL = block/point which is a Letter */
  	$this->use_arc_solr= $this->a{'use_arc_solr'};
  	
  	if ($this->use_arc_solr)
		{
		 /* Fabio Ricci (FRI) adding a bridge for SOLR
			* This bridge reacts to every add/delete triple operation in ARC */
		 $segment=$this->a{'segment'};
			
		 //FRI - ARC-SOLR BRIDGE?
		 if ($segment)
		 {
			//OPTIMIZE: THESE INCLUDE SHOULD STAY OUTSIDE THE LOOP
			$filename="$segment/app/u/SOLRinterface/arc_solr_bridge.php";$maxretries=10;
			#######################################
			for ($x=1,$updir='';$x<=$maxretries;$x++,$updir.="../")
			{/*print "<br>search '$updir$filename'";*/if (file_exists("$updir$filename")) {/*print "<br>ARC2_Class loaded '$updir$filename'";*/include_once("$updir$filename");break;}}
		 }
		 //else print "<br>NO RODIN SEGMENT GIVEN IN ARC CONFIG - no ARCSOLR BRIDGE POSSIBLE";
		} // ARC2_Class::$use_arc_solr
	}
