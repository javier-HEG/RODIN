...
class ARC2_StoreLoadQueryHandler extends ARC2_StoreQueryHandler {
  ...
  /**
	 * FRI - special enhancement - return solr document to be submitted later
	 */
  function addT($s, $p, $o, $s_type, $o_type, $o_dt = '', $o_lang = '') {
    ...
    /* triple */
    $t = array( /* FRI: store also the id's in SOLR */
      's' => $s_id = $this->getStoredTermID($s, $type_ids[$s_type], 's'),
      'p' => $p_id = $this->getStoredTermID($p, '0', 'id'),
      'o' => $o_id = $this->getStoredTermID($o, $type_ids[$o_type], 'o'),
      'o_lang_dt' => $this->getStoredTermID($o_dt . $o_lang, $o_dt ? '0' : '2', 'id'),
      'o_comp' => $o_comp= $this->getOComp($o),
      's_type' => $type_ids[$s_type], 
      'o_type' => $type_ids[$o_type],
    );
		
   
		//FRI: store/grasp even the triple id:
    $t['t'] = $triple_id = $this->getTripleID($t);
    ...
    if (($this->t_count % $this->write_buffer_size) == 0) {
      ...
      $this->checkSQLBuffers($force_write, $reset_buffers, $refresh_lock, $split_tables);
		 ...
		 /**
		  * FRI: This section grasp all the ARC internal infos and store them in SOLR
		  **/
			$solr_docs = 	$this->use_arc_solr
								? 	arc_solr_bridge_addT_prepare_solr_doc('1', $this, $g, $triple_id, $s_id,$p_id,$o_id, $s, $p, $o, $o_comp, $s_type, $o_type, $o_lang)
								:		null;
		return $solr_docs;
  } // addT
