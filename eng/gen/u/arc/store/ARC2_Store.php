  ...
  function setUp($force = 0) {
    if (($force || !$this->isSetUp()) && ($con = $this->getDBCon())) {
      ...
      $mgr->createTables();
		...
		//FRI create SOLR collection to ARC store if not existent
		if ($this->use_arc_solr)
		{
			$solr_collection=$this->a{'solr_collection'};
			arc_solr_bridge_create_collection($this,$solr_collection);
		} //create SOLR collection to ARC
  ...
  function reset($keep_settings = 0) {
    ...
    foreach ($tbls as $tbl) {
      ...
      $this->queryDB('TRUNCATE ' . $prefix . $tbl, $con);
		...
		{
			$solr_collection=$this->a{'solr_collection'};
			arc_solr_bridge_reset_collection($this,$solr_collection);
		} //create SOLR collection to ARC
  }
