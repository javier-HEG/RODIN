...
class RAMEAUengineSOLR extends RAMEAUengine
{
	protected $maxROWSinSOLR_eachEntity;
  protected $RAMEAU_SKOS_FIELDS;
	protected $bnf_rameau_namespaces;
  protected $solr_collection;
	...
	#########################
	{
		...
		$this->currentclassname='RAMEAUengineSOLR';

    $this->solr_collection = 'bnf_rameau';
		$this->setWordbinding('RAMEAU');
  ...
  public function RAMEAUengineSOLRrefine()
	{
		print '<br> hallo RAMEAUengineSOLRrefine';
	}
   ...
   * for each of action=broader,narrower,related
	...
	{	
		...
		if ($this->srcdebug) {
				if ($lang<>'fr')
 	         print "<hr>refine_skos_solr(): FORCING PASSED LANGUAGE ($lang) TO BE FRENCH! lang=fr";
		}
		$lang='fr';
		
		if ($this->srcdebug) print "<br>RAMEAUengineSOLR->refine_skos_solr($text, $q, $m, $lang) ...";
		$ok=true;
		$TERMS_RAW=array();
  ...
  # Find Terme related to $action 
    ...
    global $RODINSEGMENT;
		...
		/* Terms in RAMEAU are left as they are */
		/* Try to make them like this to SQL-Match them */
		if ($this->getSrcDebug()) print "<br>RAMEAUengineSOLR->$METHODNAME($term)...";
    ...
    // 
		if ($this->getVerbose())
		{
      list($BROADER_LABELS,$BROADER_DESC)=$RAMEAU_SKOSResult->broader;
      list($NARROWER_LABELS,$NARROWER_DESC)=$RAMEAU_SKOSResult->narrower;
      list($RELATED_LABELS,$RELATED_DESC)=$RAMEAU_SKOSResult->related;
      
      print "<br>refine_skos_solr_method RESULT SKOS ($term):";
      var_dump($RAMEAU_SKOSResult); print "<br>";
...
//       if ($nextterm) // term is non stopword
   ...
   /**
	 * RAMEAU implementation of the function, since the data is held
 ...
 protected function validateTermInOntology($term, $lang = 'en') {
		if ($this->getSrcDebug()) {
			print "<br>rameau_validate_term($term,$lang='en') USING SOLR ...";
		}
		list($suggested_term,$descriptor) = $this->checkterm_in_rameau($term,$lang);
	...
	/**
	 * Checks if $term is contained as a label in the SKOS RAMEAU Thesaurus,
	 * returns a pair (suggested term, descriptor) if yes
   * @author: Fabio Ricci
	 */
	protected function checkterm_in_rameau($term,$lang) 
  {
  	if ($this->getSrcDebug()) {
...
private function get_skos_rameau_nodes_SOLR($term,$descriptor,$m,$lang,$mode)
{
  ...
  else
  {
    ...
    list ($BROADER_LABELS,
          $NARROWER_LABELS,
          $RELATED_LABELS) 
              = $this->resolve_skos_rameau_descriptors(	$BROADER_DESC,
		                                                    $NARROWER_DESC,
		                                                    $RELATED_DESC,
		                                                    $m,
		                                                    $lang );
  ...
  if ($this->getSrcDebug())
    {
      ...
      print "<hr><br>in get_rameau_skos_nodes_SOLR():";
 ...
 */
  private function resolve_skos_rameau_descriptors(&$BROADER_DESC,&$NARROWER_DESC,&$RELATED_DESC,$m,$lang='fr')
  {
    ...
    if (($SOLRCLIENT = init_SOLRCLIENT($this->solr_collection,'solr_index_skos_namespaces system error init SOLRCLIENT')))
    {
      ...
      if ($this->getVerbose())
      {
        print "<br>resolve_skos_rameau_descriptors query=($disjunction) ...";
      }
    ...
    if ($this->getSrcDebug())
    {
      print "<br>in resolve_skos_rameau_descriptors():";
  ...
  } // resolve_skos_rameau_descriptors
 ...
 */
  private function walk_rameau_root_path($descriptor,$lang='fr',$recursion=0)
  {
    ...
    if ($this->getSrcDebug())
    {
      print "<br>walk_rameau_root_path called with lang=$lang and descriptor: ";var_dump($descriptor);print "<br>";
    }
    ...
    {
      print "<br>walk_rameau_root_path($descriptor) superated MAXRECURSION=$MAXRECURSION";
    ...
    if ($this->getSrcDebug())
    {
      print "<br>in walk_rameau_root_path($descriptor): $preflabel";
    ...
    {  
      ...
      foreach($broader as $singlebroader_descriptor)
      {
        ...
        { 
          ...
          $pathremote = $this->walk_rameau_root_path($singlebroader_descriptor,$lang,$recursion);
          // Add:
          if ($pathremote)
  ...
  private function denormalize_descriptor($RAMEAU_SOLR_DESCRIPTORS,$namespace)
  {
    if (!is_array($RAMEAU_SOLR_DESCRIPTORS)) 
    {
      return str_replace($namespace.'_',$this->namespaces[$namespace],$RAMEAU_SOLR_DESCRIPTORS);
    }
    else
    { 
      foreach($RAMEAU_SOLR_DESCRIPTORS as $RAMEAU_SOLR_DESCRIPTOR)
      {
        $NORMALIZED_DESCRIPTORS[] = str_replace($namespace.'_',$this->namespaces[$namespace],$RAMEAU_SOLR_DESCRIPTOR);
      }
