...
class RAMEAUengineSPARQL extends RAMEAUengine
{
	...
	{
		parent::__construct();
		$this->currentclassname='RAMEAUengineSPARQL';
  ...
  # Find Terme related to $action 
    ...
    global $RODINSEGMENT;
		...
		/* Terms in RAMEAU have to be considered lower case ! */
     ...
     list($labels,$descriptors) =  $this->exec_skos_node_sparql($this->get_store(),$action,$node,$lang,$lang);
      ...
      $term= $this->formatAsInThesaurus($term);
			...
			if (0 && $RODINSEGMENT<>'st' && $RODINSEGMENT <>'x' )
	...
	if ($this->getSrcDebug())
		{	
			print "<br><br><br>EXEC SKOS SPARQL $verb,($term),$SearchType,$lang_in,$lang_out <br>";
...
EOQ;
		...
		if ($this->getSrcDebug())
			print "<br><br>exec_skos_sparql($SearchType): ".str_replace("\n","<br>",htmlentities($QUERY))."<br>";
