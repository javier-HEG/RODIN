...
class STWengineSOLR extends STWengine
{
  ...
  # Find Terme related to $action 
    ...
    global $RODINSEGMENT;
		...
		if ($this->getSrcDebug()) print "<br>STWengineSOLR->$METHODNAME($term,$m)...";
...
private function get_stw_skos_nodes_SOLR($term,$descriptor,$m,$lang,$mode)
{
  ...
  else
  {
    ...
    //array_splice($NARROWER_DESC,$m);
  	...
  	{
			print "<br>B/N/R: m=($m)";
			print "<br> <b>".count($BROADER_DESC)." B descriptors: </b>"; var_dump($BROADER_DESC);
			print "<br> <b>".count($NARROWER_DESC)." N descriptors: </b>"; var_dump($NARROWER_DESC);
			print "<br> <b>".count($RELATED_DESC)." R descriptors: </b>"; var_dump($RELATED_DESC);
  	}
 ...
 */
  private function resolve_skos_stw_descriptors(&$BROADER_DESC,&$NARROWER_DESC,&$RELATED_DESC,$m,$lang='en')
  {
    ...
    if (($SOLRCLIENT = init_SOLRCLIENT($this->solr_collection,'solr_index_skos_namespaces system error init SOLRCLIENT')))
    {
      ...
      {
        //Add only different descriptors (obsolete, since array unique on descriptors before):
      ...
      foreach ($resultset as $document) 
      {
        foreach($document AS $fieldname => $value)
        {
          ...
          {
            case $NEEDED_FIELD:
							//Eval field, take every nonempty value
              $label = trim(is_array($value)?$value[0]:$value); 
            break;
            case 'id': // the descriptor
              $id=$value;
          }  
        } // each fieldname
        if ($label)
    ...
    /* reconstruct vectors from LABELS ON to give them back as they where: */
    ...
    if ($this->getSrcDebug())
    {
      print "<br><br>in resolve_skos_stw_descriptors():";
      print "<br><br>".count($BROADER_LABELS)." prepared BROADER_LABELS:<br>"; var_dump($BROADER_LABELS);
      print "<br><br>".count($NARROWER_LABELS)." prepared NARROWER_LABELS:<br>"; var_dump($NARROWER_LABELS);
      print "<br><br>".count($RELATED_LABELS)." prepared RELATED_LABELS:<br>"; var_dump($RELATED_LABELS);
    } 
