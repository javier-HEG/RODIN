...
class GNDengineSOLR extends GNDengine
{
   ...
   * for each of action=broader,narrower,related
	...
	{	
		...
		if ($this->srcdebug) print "<br>GNDengineSOLR->refine_skos_solr($text, $q, $m, $lang) ...";
		$ok=true;
		$TERMS_RAW=array();
      ...
      if (trim($text))
      {
        $GND_SKOSResult = $this->refine_skos_solr_method( trim($text),$m,$lang );
  ...
  # Find Terme related to $action 
    ...
    global $RODINSEGMENT;
		...
		{
       $GND_SKOSResult  =  $this->get_rameau_skos_nodes_SOLR(null,$descriptor,$m,$lang);
		} # node
		###########################################
      ...
      $GND_SKOSResult  = $this->get_skos_rameau_nodes_SOLR($term,null,$m,$lang);
...
/*
 * Returns an GND SKOS NODE corresponding to a SOLR search for '$term'
 */
private function get_skos_rameau_nodes_SOLR($term,$descriptor,$m,$lang)
{
  ...
  else
  {
    if ($this->getVerbose())
		{
      print "<br>GNDengineSOLR->get_rameau_skos_nodes_SOLR($term,$descriptor,$m,$lang) ...";
    }
