...
class ARC2_Reader extends ARC2_Class {
  ...
  function activate($path, $data = '', $ping_only = 0, $timeout = 0) {
    $this->setCredentials($path,'arc_reader_server_credentials');
    $this->setCredentials($path,'arc_reader_proxy_credentials');
  ...
  function setCredentials($url,$credentialtype) 
  /* $credentialtype = one of {'arc_reader_server_credentials','arc_reader_proxy_credentials'} */
  {
  	if (!$creds = $this->v($credentialtype, array(), $this->a))  return 0;
  	//if (!$creds = $this->v('arc_reader_credentials', array(), $this->a))  return 0;
  	foreach ($creds as $pattern => $cred) {
  	    // $regex = '/' . preg_replace('/([\:\/\.\?])/', '\\\\\1', $pattern) . '/';
		// FRI: The following should be DEACTIVATED, since it matches only a TCP/IP-Number
		//      if the proxy or server name is symbolical ... it should also be processed
		// if (0 && !preg_match($regex, $url)) 
		//		continue;
    	//	
      ...
      /* add header */
      if ($auth) {			
      	$AUTH_DIRECTIVE= preg_match("/proxy/",$credentialtype)? 'Proxy-Authorization':'Authorization';
        $this->setCustomHeaders("$AUTH_DIRECTIVE: " . $auth);
        /* FRI WHY ONLY ONE? */ // break;
      }
    }
  } //setCredentials
