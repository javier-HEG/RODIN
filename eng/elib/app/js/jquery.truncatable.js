				...
				if(stringLength>defaults.limit){
					...
					for(var newLimit=defaults.limit; newLimit<stringLength; newLimit++){
						...
						if(whiteSpace.test(newSplitPoint)){
							...
							$(this).html($(this).html().substr(0,setNewLimit)).append('<a class="more_'+num+'">'+defaults.more+'<a/> '+hiddenText);
							$('span.hiddenText_'+num).append(
								'<a class="close hide_'+num+'" href="" title="'+defaults.hideText+'">'+defaults.hideText+'</a>'
							);
