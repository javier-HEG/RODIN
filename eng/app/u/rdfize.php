...
###########################################################################
/**
 * Author Fabio Ricci (fabio.ricci@semweb.ch) for HEG
 * This method performs severl steps in one
 * 1. RDFize results from a search ($sid)
 * 2. Expands subjects using thesauri
...
{
	$DEBUG=0;
	global $rdfize, $listwr, $USER_ID, $reqhost, $WANT_USER_RESONANCE;
	global $WANT_RDF_STORE_INITIALIZED_AT_EVERY_SEARCH;
	global $WEBSERVICE;
	
	if ($DEBUG) {
		$countTHE = count(&$THESRCRECORDS);
		$countLOD = count(&$LODSRCRECORDS);
	...
	$added_triples 	= $count_added_triples_rdfize_extract_subjects
									+	$count_added_triples_expand_rdfize_subjects
									+ $count_triples_added_lod_subJ_doc_fetch;
	if (!$WEBSERVICE && $COUNTTRIPLES) print "<br>Globally $added_triples added triples for this rdf expansion";
