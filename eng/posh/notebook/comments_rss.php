...
/*//get the user information for feed title
$DB->getResults($rss_getUserInformation,$DB->escape($id));
$row=$DB->fetch(0);
echo "<title>".$row["long_name"]." (comments)</title>";
...
//get latest comments
$DB->getResults($commentsrss_getRssComments,$DB->escape($id),$DB->escape($notebookAccessLevel));
while ($row=$DB->fetch(0))
{
	echo "<item>";
	echo "<title><![CDATA[".$row["long_name"]." : ".substr($row["message"],0,20)."...]]></title>";
	//echo "<link><![CDATA[".__LOCALFOLDER."notebook/detail.php?id=".$id."&artid=".$row["article_id"]."#comments]]></link>";
	echo '<link><![CDATA[javascript:$p.notebook.open('.$id.',"note","'.$title.'",indef,'.$row["article_id"].',indef,"comments")]]></link>';
