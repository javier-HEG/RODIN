...
/**
 *
 *  javascript function to manage proxy
 *  
 *  file: proxy.js
 *
 */
function getProxy(v_url)
{
	v_url=_unesc(v_url);
	var l_host=getHost(v_url);
	if (__useproxy)
	{
		//check the proxy behaviour with the function provided in the .pac file
		var l_res=FindProxyForURL(v_url,l_host);
		if (l_res.indexOf("DIRECT")!=-1) return "";
		if (l_res.indexOf("PROXY")!=-1)
		{
			var l_proxy=l_res.substr(l_res.indexOf("PROXY")+6);
			if (l_proxy.indexOf(" ")!=-1) l_proxy=l_proxy.substr(0,l_proxy.indexOf(" "));
			if (l_proxy.indexOf(";")!=-1) l_proxy=l_proxy.substr(0,l_proxy.indexOf(";"));
			return l_proxy;
		}
		return "";
...
function getHost(v_url)
{
	var l_host=v_url.replace("http://","");
	if (l_host.indexOf("/")>0) l_host=l_host.substr(0,l_host.indexOf("/"));
	if (l_host.indexOf(":")>0) l_host=l_host.substr(0,l_host.indexOf(":"));
	return l_host;
}
...
function isInNet(host, pattern, mask)
{
	...
	for (var i=0; i < 4; i++)
	{
		...
		if (i != 3)result = (pattern.substring(offset1, ofs1) - 0) & (mask.substring(offset2, ofs2) - 0);
		else result = (pattern.substring(offset1) & mask.substring(offset2));
		if (result == 0)result = '*';
		m = m + result;
		if (i != 3)m = m + '.';
		offset1 = ofs1 + 1;
...
function shExpMatch(str, shexp)
{
	...
	if (shexp == '*') return true;  if (str == '' && shexp == '') return true;
	str = str.toLowerCase();
...
{
	var cur = new Date();
	if (typeof p1 == 'undefined')return false;
	else if (typeof p2 == 'undefined' || _isGmt(p2))
	{
		if ((typeof p1) == 'string')return _dateRange(1, p1, cur.getFullYear(), 31, p1, cur.getFullYear(), _isGmt(p2));
		else if (typeof p1 == 'number' && p1 > 31)return _dateRange(1, 'JAN', p1, 31, 'DEC', p1, _isGmt(p2));
		else
		{
	...
	{
		if ((typeof p1) == 'string')return _dateRange(1, p1, cur.getFullYear(), 31, p2, cur.getFullYear(), _isGmt(p3));
		else if (typeof p1 == 'number' && typeof p2 == 'number' && (p1 > 31 || p2 > 31))return _dateRange(1, 'JAN', p1, 31, 'DEC', p2, _isGmt(p3));
		else
		{
			if ((typeof p2) == 'string')
			{
	...
	else if (typeof p4 == 'undefined' || _isGmt(p4))return _dateRange(p1, p2, p3, p1, p2, p3, _isGmt(p4));
	else if (typeof p5 == 'undefined' || _isGmt(p5)){
		if (typeof p2 == 'number')return _dateRange(1, p1, p2, 31, p3, p4, _isGmt(p5));
		else return _dateRange(p1, p2, cur.getFullYear(), p3, p4, cur.getFullYear(), _isGmt(p5))
	}
	else if (typeof p6 == 'undefined')return false;
	else return _dateRange(p1, p2, p3, p4, p5, p6, _isGmt(p7));
}
function timeRange(p1, p2, p3, p4, p5, p6, p7)
{
	if (typeof p1 == 'undefined')return false;
	else if (typeof p2 == 'undefined' || _isGmt(p2))return _timeRange(p1, 0, 0, p1, 59, 59, _isGmt(p2));
	else if (typeof p3 == 'undefined' || _isGmt(p3))return _timeRange(p1, 0, 0, p2, 0, 0, _isGmt(p3));
	else if (typeof p4 == 'undefined')return false;
	else if (typeof p5 == 'undefined' || _isGmt(p5))return _timeRange(p1, p2, 0, p3, p4, 0, _isGmt(p5));
	else if (typeof p6 == 'undefined')return false;
	else return _timeRange(p1, p2, p3, p4, p5, p6, _isGmt(p7));
}
...
{
	if (typeof wd1 == 'undefined') return false;
	else if (typeof wd2 == 'undefined' || _isGmt(wd2)) return _weekdayRange(wd1, wd1, _isGmt(wd2));
	else return _weekdayRange(wd1, wd2, _isGmt(gmt));
}
