...
$p.app.menu={
	...
	*/   
    ...
    {
		$p.print('menubuttons','<div id="menuclosebutton" style="text-align: right;"><span style="margin: 5px;"><a href="#" onclick="$p.app.menu.close()">'+lg("lblClose")+' </a></span></div>');
    },
	/*
		Function: close
                                $p.app.menu.close
...
$p.app.tabs={
	...
	*/
	open: function(v_id)
	{
        ...
        if (tab[v_id] && !tab[v_id].temporary)
	...
	*/
	show:function()
	{
		...
		if ($p.app.banner.option.shown) {
			...
			if (__menuDefaultStatus != 4 && $p.app.env!="admin")
			{
				l_s+=(tab[$p.app.tabs.sel].type==1?'<a href="#" onclick=\'$p.app.menu.open("pwidget",true)\'>'+$p.img('ico_menu_add.gif',14,14,'','imgmid')+' '+lg('lblAddContent')+'</a> | ':'')
...
$p.network={
	...
	*/	
	myprofile:function()
	{
		...
		{
			$p.network.init();
			$p.app.setState("$p.network.myprofile()");
			$p.network.buildPageMenu(1);
            
			var l_s = '<div>'+$p.html.buildTitle(lg('myProfile'))+'</div>'
                + '<div class="sidebar">'
                + $p.html.roundBox('<div id="mypassword"></div>'
