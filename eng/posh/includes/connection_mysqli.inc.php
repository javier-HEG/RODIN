...
class connection
{
	...
	var $result;
	/*
	 * FRI - Adaptation to mysqli for php 5.5
		connection : Ctor
		Inputs:
	...
	*/
	...
	{
		$this->id = mysqli_connect($server, $usernamedb, $passworddb, $dbname);
		if ($this->id)
		{
	...
	*/
	function getResults($sql)
	{
		...
		$this->result = mysqli_query($this->id,$this->sql);
	...
	*/
	function fetch($type)
	{
		$l_type=($type==0)?mysqli_ASSOC:mysqli_NUM;
	...
	*/
	function execute($sql)
	{
		...
		return mysqli_query($this->id,$this->sql);
	}
	/*
	...
	*/
	...
	{
		Global $pagename;
		$errormsg=((mysqli_error()!==false)?mysqli_error():'Unknown error')."(".((mysqli_errno()!==false)?mysqli_errno():0).")";
		echo $errormsg;
		error_log("WARNING SQL : ".$this->sql." SET AN ERROR => ".$errormsg);
        mysqli_query('INSERT INTO adm_log (log,logdate,typ) VALUES ("'.$pagename.' : '.$errormsg.'",CURRENT_DATE,"O") ');
	...
	function noSqlInjection($str,$type)
	{
		...
		if ($type==2) $str=mysqli_real_escape_string($str);
		if ($type==3){
			$str=str_replace("--","",$str);
			$str=strtr($str,"=#","  ");
		}
	...
	function escape($str,$force=false)
	{
		...
		$str = mysqli_real_escape_string($this->id,$str);
		return $str;
	}
